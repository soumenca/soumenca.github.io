...
c=data['consulting']
def enquiry(subject):
 return 'mailto:soumen.ghosh@uq.edu.au?'+urlencode({'subject':subject},quote_via=quote)
offers=''.join('<article class="service-card"><p class="service-audience">'+esc(o['audience'])+'</p><h2>'+esc(o['title'])+'</h2><p>'+esc(o['description'])+'</p><ul>'+''.join('<li>'+esc(t)+'</li>' for t in o['topics'])+'</ul></article>' for o in c['offers'])
course=c['course']
consulting=heading('Work with me','Consulting &amp; Training',esc(c['intro']))+'<div class="consulting-page"><div class="service-grid">'+offers+'</div><section class="course-panel" aria-labelledby="course-title"><div><p class="status-label">'+esc(course['status'])+'</p><h2 id="course-title">'+esc(course['title'])+'</h2><p>'+esc(course['description'])+'</p><p>'+esc(course['detail'])+'</p></div><div class="course-action">'+link('Express interest in the course',enquiry('Clinical AI course — expression of interest'),'button secondary')+'</div></section><section class="consulting-contact" aria-labelledby="enquiry-title"><p class="eyebrow">Start a conversation</p><h2 id="enquiry-title">Discuss a project or training need</h2><p>'+esc(c['contact_intro'])+'</p><div class="button-row">'+link('Discuss your project or training',enquiry('Consulting or training enquiry'),'button primary')+link('View my research','research.html','text-link')+'</div><p class="contact-email">'+link('soumen.ghosh@uq.edu.au','mailto:soumen.ghosh@uq.edu.au')+'</p></section></div>'
page('consulting.html','Consulting & Training','Research consulting, industry support and teaching in medical imaging and clinical AI. Clinical AI course in development.',consulting)
